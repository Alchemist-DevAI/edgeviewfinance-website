...
const handleRequest = options => {
  ...
  return async (ctx, next) => {
    ...
    if (rootSpan && core.spanToJSON(rootSpan).op === 'http.server') {
...
async function enhanceHttpServerSpan(
  ...
  if (locals) {
    core.addNonEnumerableProperty(locals, '__sentry_wrapped__', true);
  }
  ...
  try {
    ...
    rootSpan.setAttributes({
      ...
      method,
      [node.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: 'auto.http.astro',
    });

    if (parametrizedRoute) {
      rootSpan.setAttributes({
        [node.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: 'route',
...
async function instrumentRequestStartHttpServerSpan(
  ...
  return node.withIsolationScope(isolationScope => {
    return node.continueTrace(
      ...
      async () => {
        ...
        try {
          ...
          const attributes = {
            [node.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: 'auto.http.astro',
            [node.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: source,
            [core.SEMANTIC_ATTRIBUTE_HTTP_REQUEST_METHOD]: method,
            // This is here for backwards compatibility, we used to set this here before
            method,
            url: core.stripUrlQueryAndFragment(ctx.url.href),
            ...node.httpHeadersToSpanAttributes(
              node.winterCGHeadersToDict(request.headers),
              node.getClient()?.getOptions().sendDefaultPii ?? false,
            ),
          };
            ...
            async span => {
              try {
                ...
                if (originalResponse.status) {
                  node.setHttpStatus(span, originalResponse.status);
                }
...
function getMetaTagsStr({
  ...
  const parts = [];
  if (injectTraceData) {
    parts.push(node.getTraceMetaTags());
  }
  if (parametrizedRoute) {
...
function getParametrizedRoute(
  ...
  try {
    ...
    const matchedRouteSegmentsFromManifest = routesFromManifest?.find(
      (route) => route?.routeData?.route === rawRoutePattern,
    )?.routeData?.segments;
